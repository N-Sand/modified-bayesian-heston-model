...
data {
    ...
    array[T] int days;          // Days since start (for GP kernel)
}
...
transformed parameters {
    ...
    vector[T] f;               // GP mean function
    {
        ...
        for (i in 1:T) {
        Ker[i, i] = square(gp_amplitude) + gp_noise;
        for (j in (i+1):T) {
            Ker[i, j] = square(gp_amplitude) * exp(-0.5 * square((days[i] - days[j]) / gp_length_scale));
            Ker[j, i] = Ker[i, j];
        }
        }
    ...
    log_v[1] = log_v1;
    for (t in 2:T) {
        real drift = kappa * (log_theta + f[t] - log_v[t - 1]);
...
model {
    // Priors
    // GP priors
    gp_length_scale ~ normal(20.0, 0.5);  // Centered at 20 days
    gp_amplitude ~ normal(0.0, 2);  // Small amplitude to limit influence
    gp_noise ~ normal(1, 0.1);  // Modest noise, tight prior
    
    // Standard normal prior for latent values
    eta ~ std_normal();
...
generated quantities {
    ...
    for (t in 1:T_future) {
        ...
        real next_log_v = last_log_v + kappa * (log_theta + f[T] - last_log_v) + sigma * next_eps_v + seasonal_effect_future[t];
